...
void RemoteControl_PC_Update(void)
{
	...
	// }
	/*鼠标左键发射控制*/
	if (((ControlMes.AutoAimFlag == 1 && GetKeyMouseAction(KEY_B, KeyAction_PRESS)) ||(ControlMes.AutoAimFlag == 0 && (GetKeyMouseAction(MOUSE_Left, KeyAction_PRESS) || GetKeyMouseAction(MOUSE_Left, KeyAction_LONG_PRESS)))) &&ControlMes.fric_Flag == 1)
	{
		ControlMes.shoot_state = RC_SW_UP;
		Dial_Data.Shoot_Mode = Continuous_Shoot;
		Shoot_Data.Shoot_Switch = 1;
